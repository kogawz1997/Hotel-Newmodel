...
type ProviderItem = { id: OAuthProvider | 'line'; label: string; bg: string; text: string; icon: React.ReactNode; soon?: boolean };
              ...
              {mode === 'forgot' ? (
                ...
                <>
                  ...
                  <div className="grid grid-cols-2 gap-2.5 mb-5">
                    {PROVIDERS.map(p => (
                      p.soon ? (
                        <div key={p.id} className={`${p.bg} ${p.text} flex items-center justify-center gap-2 py-2.5 px-3 rounded-xl text-sm font-medium opacity-50 cursor-not-allowed relative`}>
                          {p.icon}
                          <span>{p.label}</span>
                          <span className="absolute -top-1.5 -right-1.5 bg-[#2A2522] text-white text-[9px] font-bold px-1.5 py-0.5 rounded-full leading-none">เร็วๆ นี้</span>
                        </div>
                      ) : (
                        <button key={p.id} onClick={() => handleSocial(p.id as OAuthProvider)}
