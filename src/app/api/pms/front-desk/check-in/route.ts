...
const schema = z.object({
  ...
  action: z.string().max(80).optional().default('pms.front_desk_check_in'),
...
export async function GET(request: Request) {
  ...
  return NextResponse.json(buildAuditEnvelope('pms.front_desk_check_in.read', { hotelId: (ctx as any).hotelId || null, status: 'ready' }));
}
...
export async function POST(request: Request) {
  ...
  return NextResponse.json(buildAuditEnvelope('pms.front_desk_check_in.write', { hotelId: ctx.hotelId, ...parsed.data }));
}
