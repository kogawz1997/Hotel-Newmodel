...
const schema = z.object({
  ...
  action: z.string().max(80).optional().default('pms.housekeeping_tasks'),
...
export async function GET(request: Request) {
  ...
  return NextResponse.json(buildAuditEnvelope('pms.housekeeping_tasks.read', { hotelId: (ctx as any).hotelId || null, status: 'ready' }));
}
...
export async function POST(request: Request) {
  ...
  return NextResponse.json(buildAuditEnvelope('pms.housekeeping_tasks.write', { hotelId: ctx.hotelId, ...parsed.data }));
}
