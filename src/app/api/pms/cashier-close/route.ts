...
const schema = z.object({
  ...
  action: z.string().max(80).optional().default('pms.cashier_close'),
...
export async function GET(request: Request) {
  ...
  return NextResponse.json(buildAuditEnvelope('pms.cashier_close.read', { hotelId: (ctx as any).hotelId || null, status: 'ready' }));
}
...
export async function POST(request: Request) {
  ...
  return NextResponse.json(buildAuditEnvelope('pms.cashier_close.write', { hotelId: ctx.hotelId, ...parsed.data }));
}
