...
const schema = z.object({
  ...
  action: z.string().max(80).optional().default('pms.folio_refund'),
...
export async function GET(request: Request) {
  ...
  return NextResponse.json(buildAuditEnvelope('pms.folio_refund.read', { hotelId: (ctx as any).hotelId || null, status: 'ready' }));
}
...
export async function POST(request: Request) {
  ...
  return NextResponse.json(buildAuditEnvelope('pms.folio_refund.write', { hotelId: ctx.hotelId, ...parsed.data }));
}
