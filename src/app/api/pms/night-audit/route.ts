...
const schema = z.object({
  ...
  action: z.string().max(80).optional().default('pms.night_audit'),
...
export async function GET(request: Request) {
  ...
  return NextResponse.json(buildAuditEnvelope('pms.night_audit.read', { hotelId: (ctx as any).hotelId || null, status: 'ready' }));
}
...
export async function POST(request: Request) {
  ...
  return NextResponse.json(buildAuditEnvelope('pms.night_audit.write', { hotelId: ctx.hotelId, ...parsed.data }));
}
