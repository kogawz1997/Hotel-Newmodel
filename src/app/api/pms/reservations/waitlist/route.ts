...
const schema = z.object({
  ...
  action: z.string().max(80).optional().default('pms.waitlist'),
...
export async function GET(request: Request) {
  ...
  return NextResponse.json(buildAuditEnvelope('pms.waitlist.read', { hotelId: (ctx as any).hotelId || null, status: 'ready' }));
}
...
export async function POST(request: Request) {
  ...
  return NextResponse.json(buildAuditEnvelope('pms.waitlist.write', { hotelId: ctx.hotelId, ...parsed.data }));
}
