...
const schema = z.object({
  ...
  action: z.string().max(80).optional().default('pms.reservation_calendar'),
...
export async function GET(request: Request) {
  ...
  return NextResponse.json(buildAuditEnvelope('pms.reservation_calendar.read', { hotelId: (ctx as any).hotelId || null, status: 'ready' }));
}
...
export async function POST(request: Request) {
  ...
  return NextResponse.json(buildAuditEnvelope('pms.reservation_calendar.write', { hotelId: ctx.hotelId, ...parsed.data }));
}
