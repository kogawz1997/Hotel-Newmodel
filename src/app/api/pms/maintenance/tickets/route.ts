...
const schema = z.object({
  ...
  action: z.string().max(80).optional().default('pms.maintenance_tickets'),
...
export async function GET(request: Request) {
  ...
  return NextResponse.json(buildAuditEnvelope('pms.maintenance_tickets.read', { hotelId: (ctx as any).hotelId || null, status: 'ready' }));
}
...
export async function POST(request: Request) {
  ...
  return NextResponse.json(buildAuditEnvelope('pms.maintenance_tickets.write', { hotelId: ctx.hotelId, ...parsed.data }));
}
