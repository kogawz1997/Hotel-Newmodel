...
const schema = z.object({
  ...
  action: z.string().max(80).optional().default('pms.maintenance_escalation'),
...
export async function GET(request: Request) {
  ...
  return NextResponse.json(buildAuditEnvelope('pms.maintenance_escalation.read', { hotelId: (ctx as any).hotelId || null, status: 'ready' }));
}
...
export async function POST(request: Request) {
  ...
  return NextResponse.json(buildAuditEnvelope('pms.maintenance_escalation.write', { hotelId: ctx.hotelId, ...parsed.data }));
}
