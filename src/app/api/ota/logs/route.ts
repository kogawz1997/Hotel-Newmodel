...
const schema = z.object({
  ...
  action: z.string().max(80).optional().default('ota.sync_logs'),
...
export async function GET(request: Request) {
  ...
  return NextResponse.json(buildAuditEnvelope('ota.sync_logs.read', { hotelId: (ctx as any).hotelId || null, status: 'ready' }));
}
...
export async function POST(request: Request) {
  ...
  return NextResponse.json(buildAuditEnvelope('ota.sync_logs.write', { hotelId: ctx.hotelId, ...parsed.data }));
}
