...
const schema = z.object({
  ...
  action: z.string().max(80).optional().default('ota.conflict_resolution'),
...
export async function GET(request: Request) {
  ...
  return NextResponse.json(buildAuditEnvelope('ota.conflict_resolution.read', { hotelId: (ctx as any).hotelId || null, status: 'ready' }));
}
...
export async function POST(request: Request) {
  ...
  return NextResponse.json(buildAuditEnvelope('ota.conflict_resolution.write', { hotelId: ctx.hotelId, ...parsed.data }));
}
