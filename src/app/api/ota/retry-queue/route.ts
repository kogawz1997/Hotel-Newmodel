...
const schema = z.object({
  ...
  action: z.string().max(80).optional().default('ota.retry_queue'),
...
export async function GET(request: Request) {
  ...
  return NextResponse.json(buildAuditEnvelope('ota.retry_queue.read', { hotelId: (ctx as any).hotelId || null, status: 'ready' }));
}
...
export async function POST(request: Request) {
  ...
  return NextResponse.json(buildAuditEnvelope('ota.retry_queue.write', { hotelId: ctx.hotelId, ...parsed.data }));
}
