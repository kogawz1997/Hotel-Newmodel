...
const schema = z.object({
  ...
  action: z.string().max(80).optional().default('ota.mapping'),
...
export async function GET(request: Request) {
  ...
  return NextResponse.json(buildAuditEnvelope('ota.mapping.read', { hotelId: (ctx as any).hotelId || null, status: 'ready' }));
}
...
export async function POST(request: Request) {
  ...
  return NextResponse.json(buildAuditEnvelope('ota.mapping.write', { hotelId: ctx.hotelId, ...parsed.data }));
}
