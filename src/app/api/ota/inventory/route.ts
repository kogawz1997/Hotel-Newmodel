...
const schema = z.object({
  ...
  action: z.string().max(80).optional().default('ota.inventory_sync'),
...
export async function GET(request: Request) {
  ...
  return NextResponse.json(buildAuditEnvelope('ota.inventory_sync.read', { hotelId: (ctx as any).hotelId || null, status: 'ready' }));
}
...
export async function POST(request: Request) {
  ...
  return NextResponse.json(buildAuditEnvelope('ota.inventory_sync.write', { hotelId: ctx.hotelId, ...parsed.data }));
}
