/**
 * TrueMoney Wallet Payment via Omise
 * Guest กรอกเบอร์โทรที่ผูก TrueMoney → Omise ส่ง OTP → รับชำระ
 *
 * Flow:
 * 1. POST พร้อม reservationId + amount + phoneNumber
 * 2. สร้าง Omise source type=truemoney
 * 3. สร้าง Omise charge → status=pending (รอ OTP)
...
const schema = z.object({
  ...
  phoneNumber: z.string().regex(/^0[689]\d{8}$/, 'กรุณากรอกเบอร์โทรที่ถูกต้อง (เช่น 0812345678)'),
...
export async function POST(request: NextRequest) {
  ...
  const limited = await rateLimit(request, 'payments.truemoney', 10, 60_000);
  ...
  const { reservationId, amount, currency, phoneNumber, description } = parsed.data;
  ...
  // Create TrueMoney source — Omise source type is 'truemoney'
  ...
  if (!sourceRes.ok) {
    ...
    return NextResponse.json(
      { error: err?.message || 'ไม่สามารถสร้าง TrueMoney source ได้' },
      { status: 502 }
    );
  }
  ...
  if (!chargeRes.ok) {
    ...
    return NextResponse.json(
      { error: err?.message || 'ไม่สามารถสร้าง TrueMoney charge ได้' },
      { status: 502 }
    );
  }
  ...
  if (payErr) {
    logger.error('TrueMoney payment save failed', { error: payErr.message });
  }

  await admin.from('audit_logs').insert({
    hotel_id: reservation.hotel_id,
    action: 'payment.truemoney.created',
    entity_type: 'payment',
