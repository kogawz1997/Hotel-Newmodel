...
const schema = z.object({
  ...
  action: z.string().max(80).optional().default('reviews.moderation'),
...
export async function GET(request: Request) {
  ...
  return NextResponse.json(buildAuditEnvelope('reviews.moderation.read', { hotelId: (ctx as any).hotelId || null, status: 'ready' }));
}
...
export async function POST(request: Request) {
  ...
  return NextResponse.json(buildAuditEnvelope('reviews.moderation.write', { hotelId: ctx.hotelId, ...parsed.data }));
}
