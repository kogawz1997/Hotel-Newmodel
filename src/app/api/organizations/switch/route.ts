...
const schema = z.object({
  ...
  action: z.string().max(80).optional().default('organizations.switch'),
...
export async function GET(request: Request) {
  ...
  return NextResponse.json(buildAuditEnvelope('organizations.switch.read', { hotelId: (ctx as any).hotelId || null, status: 'ready' }));
}
...
export async function POST(request: Request) {
  ...
  return NextResponse.json(buildAuditEnvelope('organizations.switch.write', { hotelId: ctx.hotelId, ...parsed.data }));
}
