...
const schema = z.object({
  ...
  action: z.string().max(80).optional().default('onboarding.setup_checklist'),
...
export async function GET(request: Request) {
  ...
  return NextResponse.json(buildAuditEnvelope('onboarding.setup_checklist.read', { hotelId: (ctx as any).hotelId || null, status: 'ready' }));
}
...
export async function POST(request: Request) {
  ...
  return NextResponse.json(buildAuditEnvelope('onboarding.setup_checklist.write', { hotelId: ctx.hotelId, ...parsed.data }));
}
