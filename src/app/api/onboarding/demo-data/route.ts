...
const schema = z.object({
  ...
  action: z.string().max(80).optional().default('onboarding.demo_data'),
...
export async function GET(request: Request) {
  ...
  return NextResponse.json(buildAuditEnvelope('onboarding.demo_data.read', { hotelId: (ctx as any).hotelId || null, status: 'ready' }));
}
...
export async function POST(request: Request) {
  ...
  return NextResponse.json(buildAuditEnvelope('onboarding.demo_data.write', { hotelId: ctx.hotelId, ...parsed.data }));
}
