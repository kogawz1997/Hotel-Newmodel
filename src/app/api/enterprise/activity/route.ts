...
const schema = z.object({
  ...
  action: z.string().max(80).optional().default('enterprise.activity_center'),
...
export async function GET(request: Request) {
  ...
  return NextResponse.json(buildAuditEnvelope('enterprise.activity_center.read', { hotelId: (ctx as any).hotelId || null, status: 'ready' }));
}
...
export async function POST(request: Request) {
  ...
  return NextResponse.json(buildAuditEnvelope('enterprise.activity_center.write', { hotelId: ctx.hotelId, ...parsed.data }));
}
