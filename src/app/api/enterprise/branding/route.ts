...
const schema = z.object({
  ...
  action: z.string().max(80).optional().default('enterprise.branding'),
...
export async function GET(request: Request) {
  ...
  return NextResponse.json(buildAuditEnvelope('enterprise.branding.read', { hotelId: (ctx as any).hotelId || null, status: 'ready' }));
}
...
export async function POST(request: Request) {
  ...
  return NextResponse.json(buildAuditEnvelope('enterprise.branding.write', { hotelId: ctx.hotelId, ...parsed.data }));
}
