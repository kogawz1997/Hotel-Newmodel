...
const schema = z.object({
  ...
  action: z.string().max(80).optional().default('enterprise.organization_hierarchy'),
...
export async function GET(request: Request) {
  ...
  return NextResponse.json(buildAuditEnvelope('enterprise.organization_hierarchy.read', { hotelId: (ctx as any).hotelId || null, status: 'ready' }));
}
...
export async function POST(request: Request) {
  ...
  return NextResponse.json(buildAuditEnvelope('enterprise.organization_hierarchy.write', { hotelId: ctx.hotelId, ...parsed.data }));
}
