...
const schema = z.object({
  ...
  action: z.string().max(80).optional().default('enterprise.permissions'),
...
export async function GET(request: Request) {
  ...
  return NextResponse.json(buildAuditEnvelope('enterprise.permissions.read', { hotelId: (ctx as any).hotelId || null, status: 'ready' }));
}
...
export async function POST(request: Request) {
  ...
  return NextResponse.json(buildAuditEnvelope('enterprise.permissions.write', { hotelId: ctx.hotelId, ...parsed.data }));
}
