...
const schema = z.object({
  ...
  action: z.string().max(80).optional().default('enterprise.cross_property_report'),
...
export async function GET(request: Request) {
  ...
  return NextResponse.json(buildAuditEnvelope('enterprise.cross_property_report.read', { hotelId: (ctx as any).hotelId || null, status: 'ready' }));
}
...
export async function POST(request: Request) {
  ...
  return NextResponse.json(buildAuditEnvelope('enterprise.cross_property_report.write', { hotelId: ctx.hotelId, ...parsed.data }));
}
