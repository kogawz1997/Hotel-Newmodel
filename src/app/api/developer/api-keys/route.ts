...
const schema = z.object({
  ...
  action: z.string().max(80).optional().default('developer.api_keys'),
...
export async function GET(request: Request) {
  ...
  return NextResponse.json(buildAuditEnvelope('developer.api_keys.read', { hotelId: (ctx as any).hotelId || null, status: 'ready' }));
}
...
export async function POST(request: Request) {
  ...
  return NextResponse.json(buildAuditEnvelope('developer.api_keys.write', { hotelId: ctx.hotelId, ...parsed.data }));
}
