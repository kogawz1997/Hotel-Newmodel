...
const schema = z.object({
  ...
  action: z.string().max(80).optional().default('developer.webhook_subscriptions'),
...
export async function GET(request: Request) {
  ...
  return NextResponse.json(buildAuditEnvelope('developer.webhook_subscriptions.read', { hotelId: (ctx as any).hotelId || null, status: 'ready' }));
}
...
export async function POST(request: Request) {
  ...
  return NextResponse.json(buildAuditEnvelope('developer.webhook_subscriptions.write', { hotelId: ctx.hotelId, ...parsed.data }));
}
