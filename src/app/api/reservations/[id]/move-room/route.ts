...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data, error } = await ctx.supabase.from('reservations')
    ...
    .select()
    .single();
  if (error || !data) return error ? apiError(error) : NextResponse.json({ error: 'Room move failed' }, { status: 500 });
  ...
  await ctx.supabase.from('audit_logs').insert({ hotel_id: reservation.hotel_id, user_id: ctx.user?.id || null, action: 'reservation.room_moved', entity_type: 'reservation', entity_id: id, changes: redactPii({ oldRoomId, newRoomId: parsed.data.roomId, reason: parsed.data.reason }) });
