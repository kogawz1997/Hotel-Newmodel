...
export async function POST(request: Request) {
  ...
  const eventId = event.id || `${event.key}_${event.data?.id}`;
  const { data: processed } = await supabase.from('audit_logs')
    .select('id').eq('action', 'webhook.omise').eq('entity_id', eventId).maybeSingle();
  if (processed) return NextResponse.json({ received: true, duplicate: true });
  await supabase.from('audit_logs').insert({
    action: 'webhook.omise', entity_type: 'payment', entity_id: eventId,
    changes: { event_type: event.key, processed_at: new Date().toISOString() },
  });
