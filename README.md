...
- Repo-only hardening items in the latest pass: `next-env.d.ts` checked in, OTA worker cron added to `vercel.json`, CI strengthened to run `check`, `reality:check`, and `build`
...
- ✅ Pull/push sync cron endpoint: `GET /api/cron/ota-sync`
...
- ✅ Trial expiry: `/api/cron/trial-expire`
- ✅ Pending-payment expiry: `/api/cron/expire-pending`
- ✅ Billing reconcile: `/api/cron/billing-reconcile`
- ✅ Billing retry follow-up: `/api/cron/billing-retry`
- ✅ Reliability sweep / DLQ recovery: `/api/cron/reliability-sweep`
- ✅ Abandoned booking recovery email: `/api/cron/abandoned-booking-recovery`
- ✅ OTA pull sync: `/api/cron/ota-sync`
- ✅ OTA queue processor: `/api/ota/process`

> หมายเหตุ: ทุก cron ต้องใช้ `CRON_SECRET` และยังต้องตรวจผลจริงหลัง deploy บน Vercel/production
