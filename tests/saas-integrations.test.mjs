...
const required = [
  ...
  'supabase/migrations/02_operations.sql',
];
...
assert.ok(vercel.crons.some(c => c.path === '/api/cron/abandoned-booking-recovery'), 'abandoned-booking-recovery cron missing');
assert.ok(vercel.crons.some(c => c.path === '/api/cron/ota-sync'), 'ota-sync cron missing');
