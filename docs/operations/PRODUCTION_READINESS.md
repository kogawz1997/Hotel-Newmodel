...
- Added `next-env.d.ts` to the repo so fresh environments do not depend on first-build file generation.
- Added `/api/ota/process` to `vercel.json` cron configuration so OTA queue processing is scheduled in production.
...
The code is now safer and more complete, but this exported snapshot still needs a real `package-lock.json` regenerated in a connected Node 20 environment before `npm ci` and `next build` can be re-verified end to end. Provider integrations also cannot be certified from source code alone. Real credentials and sandbox/production provider reviews are mandatory.
