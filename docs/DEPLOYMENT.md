...
หลัง deploy ให้ตรวจว่า `vercel.json` ถูกใช้งานพร้อม cron เหล่านี้:

- `/api/cron/trial-expire`
- `/api/cron/expire-pending`
- `/api/cron/billing-reconcile`
- `/api/cron/billing-retry`
- `/api/cron/reliability-sweep`
- `/api/cron/abandoned-booking-recovery`
- `/api/cron/ota-sync`
- `/api/ota/process`

ทุก endpoint ต้องอาศัย `CRON_SECRET` ใน production environment
