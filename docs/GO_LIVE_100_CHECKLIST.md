...
## Repo preparation status

- [x] `next-env.d.ts` committed
- [x] OTA sync cron configured in `vercel.json`
- [x] Billing / reliability / recovery cron jobs configured in `vercel.json`
- [x] Static hardening tests pass in current snapshot
- [ ] `package-lock.json` regenerated and committed from a connected install environment
